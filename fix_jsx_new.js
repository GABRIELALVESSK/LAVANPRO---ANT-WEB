...
if (start !== -1 && end !== -1 && start < end) {
    ...
    text = text.replace(/class=/g, 'className=')
        ...
        .replace(/<br >/g, '<br />')
        .replace(/onClick="toggleAccordion\((.*?)\)"/g, 'onClick={() => toggleAccordion(\'$1\')}');
    ...
    // fix unclosed p tag containing div
    text = text.replace(/<p className=\"text-brand-muted\" ><div>(.*?)<\/div><\/p>/g, '<div className="text-brand-muted" >$1</div>');
...
export default function LandingPage() {
    ...
    console.log('Successfully updated app/page.tsx with NEW stitch HTML!');
} else {
