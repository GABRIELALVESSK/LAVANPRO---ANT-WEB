 ...
 * Agora com suporte a persist├¬ncia imediata.
 */
    ...
    try {
        ...
        for (const localKey of keysToSync) {
            ...
            if (error) {
                ...
                console.log(`[DataSync] Ô£à ${serverKey} enviado para nuvem`);
            }
        }
    } catch (err) {
    ...
    try {
        ...
        for (const localKey of SYNC_KEYS) {
            ...
            if (data !== null) {
                ...
                // Apenas sobrescreve se houver mudan├ºas reais e n├úo for um dado vazio vindo do servidor quando o local tem algo
                if (serverString !== rawLocal && serverString !== '[]' && serverString !== '{}') {
                    ...
                    // SE o servidor est├í vazio mas o local tem dados (ex: nova instala├º├úo que ainda n├úo subiu nada)
                    // N├âO sobrescrevemos o local com vazio. Em vez disso, deixamos o local como est├í
                    // para que o sistema possa funcionar offline/cache e subir os dados no pr├│ximo push.
...
export async function syncSave(key: SyncKey, data: any) {
    ...
    // 1. Salva no localStorage como cache secund├írio (opcional, mas evita stale data no read inicial)
    try {
    ...
    // 4. Se for sucesso, podemos for├ºar um refresh no provider ou deixar o Realtime agir
    return result;
}
